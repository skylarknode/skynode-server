...
const   express = require('express'),
        ...
        async = require('async'),
        nconf = require('nconf'),
        slangx = require('skylark-langx').langx,
        path = require('path');
...
SkyServer.prototype.activate = function(params) {
    var self = this,
        app = this._app;
    ...
    var avtModPath = app.ncpath("activater");
    if (avtModPath) {
        require(avtModPath)(app);
    }
...
SkyServer.prototype.init = function(callback) {
    ...
    function _init(next) {
        ...
        if (app.nconf("system:private")){
            app.use(require('express-basic-auth')(app.nconf("system:private")));    
        }
    ...
    async.series([
        function(next) {
            console.log("init.1");
            self._fireHook("server.initing",{},next);
        },
        _init,
        function(next) {
            console.log("init.3");
            self._fireHook("server.inited",{},next);
        }    
    ],function(){
        callback && callback();
    });
...
SkyServer.prototype.start = function(callback) {
    ...
    async.series([
        function(next) {
            console.log("start.1");
            self._fireHook("server.starting",{},next);
        },
        ...
        function(next){
            ...
            self._fireHook("server.started",{},next)
        }
    ],function(){
        console.log("start.4");
        callback && callback();
