...
function SkyServer(params) {
    ...
    app.ncpath = function(key) {
        ...
        if (slangx.isString(value)) {
            return path.join(this.nconf("basedir"),value);
        }
    }
...
SkyServer.prototype.init = function(callback) {
    ...
    function _init(next) {
        ...
        app.store = (function (options) {
            if (options) {
                options.basedir = app.get("basedir");
        ...
        app.set("basedir", app.nconf("basedir"));

        try {
            app.set('version', require(path.join(app.get('basedir'),'package')).version);
        } catch (e) {
