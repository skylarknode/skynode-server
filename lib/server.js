...
SkyServer.prototype.init = function(config) {
    ...
    app.mailer = (function (mail) {
        if (mail) {
            return require("./email").createMailer(mail.setting,mail.smtp);
        } 
    })(config.mail);
    ...
    app.emit('before:init:restapilugin', app);
    this._initRestApiPlugin(app, config.restapi);
...
SkyServer.prototype._initHttpPlugin = function(app, http) {
    ...
    if (http.flash) {
        require('./http/flash')(app, http.flash);
    }
...
SkyServer.prototype._initRestApiPlugin = function(app, rests) {
    if (rests.cors) {
        require('./restapi/cors')(app, rests.cors);
    }
    if (rests.jsonp) {
        require('./restapi/jsonp')(app, rests.jsonp);
    }
    if (rests.ajax) {
        require('./restapi/ajax')(app, rests.ajax);
    }
};
