...
function csurf (options) {
  ...
  return function csrf (req, res, next) {
    ...
    if (!secret) {
      ...
      console.log("secret2:" + secret +",sessionKey:" + sessionKey);
...
module.exports = function(app, options) {
    ...
    app.use(function(req, res, next) {
        ...
        if (always[req.method]) {
            ...
            console.log("ignore:");
            console.dir(ignore);
            console.log("req.cors:" + req.cors);
            ignore.forEach(function(matcher) {
                console.log("ignore.matcher:" + matcher+"," + typeof matcher );

                if (typeof matcher === 'string') {
                    if ((matcher == "cors") && req.cors) {
                        skipCSRF = true;
                    } else  if (matcher === url.pathname) {
                        skipCSRF = true;
                    }
                } else {
